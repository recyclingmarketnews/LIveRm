...
<p style="padding:0px 30px;"><span class="fa fa-file"></span> ` +
      escapeHtml(fileName) +
      `</p>
</div>
`
...
<div class="image-file chat-image" style="background-image: url('` +
      imgURL +
      `');"></div>
<p><span class="fa fa-file-image"></span> ` +
      escapeHtml(fileName) +
      `</p>
</div>
`
...
function errorMessageCard(id) {
  ...
  messagesContainer
    ...
    .prepend('<span class="fa fa-exclamation-triangle"></span>');
}
  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $("#seletedusername").html(data.user_name);
        $(".info-name").html(data.user_name);
...
channel.bind("client-seen", function (data) {
  if (data.from_id == getMessengerId() && data.to_id == auth_id) {
    if (data.seen == true) {
      $(".message-time")
        .find(".fa-check")
        .before('<span class="fa fa-check-square-o seen"></span> ');
