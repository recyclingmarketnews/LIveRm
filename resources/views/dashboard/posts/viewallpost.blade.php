...
@section('content')
@if(Request::routeIs('allactivepost'))
        <script>
...
<div class="page-content">
    <div class="container-fluid">
        ...
        <form action="{{ URL::to('allactivepostt') }}" method="post" class="mainfilter dvStats" >
            @csrf
        ...
        <div class="row mt-2" style="background:<?php if($mode == 'light'){ echo '#fff'; }else{ echo '##061e2c'; } ?>; text-align:center;padding: 7px 0px;">
            <div class="col-xl-1 col-lg-1 btn-danger">
                 <a style="color: white;" href="{{ URL::to('allactivepost') }}" class="btn ">Clear</a>
            </div>
            <div class="col-xl-3 col-lg-3 forfilterm">
                <div class="m-0 form-group">
                    <select name="rating" class="form-select" id="view_type_sorting" aria-haspopup="true" aria-expanded="false">
                        <option style="color:#495057;" value="all" @if('all' == $rating) selected @endif>Rating</option>
                        <option value="1" @if('1' == $rating) selected @endif>1 Star</option> 
                        <option value="2"  @if('2' == $rating) selected @endif>2 Stars</option>  
                        <option value="3" @if('3' == $rating) selected @endif>3 Stars</option>  
                        <option value="4" @if('4' == $rating) selected @endif>4 Stars</option> 
                        <option value="5" @if('5' == $rating) selected @endif>5 Stars</option> 
                    </select>
                </div>
            </div>
            ...
            <div class="col-xl-3 col-lg-3 forfilterm">
                <div class="m-0 form-group">
                    <select name="category" style=" font-family:FontAwesome;" class="form-select">
                        <option value="all" @if('all' == $country) selected @endif>Category</option>
                        
                        @forelse($cats as $key)
                            <option value="{{ $key->id }}" @if($key->id == $category) selected @endif>{{ $key->name }}</option>
                        @empty
                        @endforelse
                    </select>
                </div>
            ...
            <div class="col-xl-1 col-lg-1">
                 <a href="{{ URL::to('addpost') }}" class="btn btn-dark hideaddnews">+ Post</a>
            </div>
        </div>            
        </form>
        <div class="row">
                     ...
                     <?php $cid = $key->category_id; $catdata = DB::table('product_category')->where('id',$cid)->first(); ?>
                          <div class="col-xl-4 col-lg-4 col-s-12 col-xs-12 mb-2" style="padding-right:0px;">
                            ...
                            <div style="position: absolute;top:3px;right: 8px; padding: 8px;" data-id="{{ $key->id }}">
                                    <i id="like{{$key->id}}" class="fa fa-thumbs-up font-size-22 {{ \App\Helpers\Helper::UserHasLikePost($key->id) ? 'like-post' : 'notlike-post' }}"></i>
                            </div>
                          </div>
                    @empty
                                    ...
                                    <div class="col-lg-12" style="padding-left: 0;">
                                        <div>
                                            ...
                                            <div class="d-flex flex-row" style="    margin-top: 0.75rem!important; font-size:10px"> 
                                               ...
                                               <a href="#"><span class="badge badge-soft-warning  font-size-10" style="margin-left: 10px;" >{{ $key->country }}</span></a>
                                                </span>
                                                </div>
                                            </div>
                ...
                @forelse($list as $key)
                           ...
                           <div class="me-2 p-2 results" style="width:100%;  padding-bottom: 5px !important;">
                                <div class="row">
                                    ...
                                    <div class="col-lg-12" style="padding-left: 0;">
                                        <div>
                                           <a style="color: gray;font-weight: bold;font-size: 14px;" href="{{ URL::to('viewprofile/'.$userdata->id) }}">{{ $userdata->fname }} {{ $userdata->lname }}</a>
                                            <div>
                                                
                                                <div class="d-flex justify-content-between">
                                                    <a  style="color:<?php if($mode == 'light'){ echo '#464545'; }else{ echo '#fff'; } ?>;font-size: 15px;font-weight: bold;margin-right: 5px;line-height: 1.1;" href="{{ URL::to('singlenews/'.$key->id) }}" class="mb-1">{{$key->heading}}</a> 
                                                    <img src="{{ asset('uploads/post/'.$key->image) }}" style="width: 50px;height: auto;border-radius: 5px;" class="img-fluid" />
                                                </div>
            ...
            <div class="col-xl-3 col-lg-3 col-md-12 col-sm-12">
                ...
                @foreach($exchangeRateswatchend as $key => $value)
                <div class="owl-item" style="margin-top: 10px;">
                    <div class="pricing-item item" style="display: flex;height: 65px;color: white;background-image: linear-gradient(360deg, #003032, #1a4547); padding: 10px;">
                        <div style="width: 50%;">
                            <p style="padding: 0;margin: 0;font-size: 16px;">USD/{{ $key }}
                           ...
                           <?php $colorsss = '#82cc8d'; $charup='inline'; $chardown='inline';
                            ...
                            <a style="font-size: 10px;padding: 1px 4px;margin: auto; color:white; background:#fc931d;" href="{{ URL::to('addtowatchlistremove/'.$key) }}">Remove</a>
                        </div>
                        <div style="width: 50%;text-align: right;">
                            <p style="padding: 0;margin: 0;font-size: 16px;">{{ round($value,2) }}</p>
                            ...
                            ?>
                            <p style="font-size: 10px; color:<?php echo $color; ?>; margin: 0;">{{  round($perc,4) }}%</p>
                        </div>
                        </div>
                    </div>
                @endforeach
                ...
                @foreach($exchangeRatesend as $key => $value)
                <?php 
                    $cur = $key;
                    ...
                    <div class="pricing-item item" style="display: flex;height: 65px;color: white;background-image: linear-gradient(360deg, #003032, #1a4547); padding: 10px;">
                        <div style="width: 50%;">
                           ...
                           <?php $colorsss = '#82cc8d'; $charup='inline'; $chardown='inline';
                            ...
                            <span class="blinkdown" style="display:<?php echo $chardown; ?>; color: <?php echo $colorsss; ?>;">▼</span></p>
                            @if($curcheckexist == '')
                            <a style="color:white; font-size: 10px;padding: 1px 4px;margin: auto;background: #2daae1;" href="{{ URL::to('addtowatchlist/'.$key) }}">Add to watchlist</a>
                            @else
                            <a style="font-size: 10px;padding: 1px 4px;margin: auto; color:white; background:#fc931d;" href="{{ URL::to('addtowatchlistremove/'.$key) }}">Remove</a>
                            @endif
                        </div>
...
$(".listview").click(function() {
    ...
    $.ajax({
        type:"post",
        url:'{{URL::to('savepostview')}}',
        data: { "_token": "{{ csrf_token() }}","view": "list"},
        datatype:"html",
...
$(".gridview").click(function() {
    ...
    $.ajax({
        type:"post",
        url:'{{URL::to('savepostview')}}',
        data: { "_token": "{{ csrf_token() }}","view": "grid"},
        datatype:"html",
        success:function(data)
        {
