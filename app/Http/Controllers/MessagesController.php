...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            ...
            if($fetch){
                $userAvatar = asset('uploads/userimg/'.$fetch->image);
            }
        }
    ...
    public function send(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array($file->getClientOriginalExtension(), $allowed)) {
                    ...
                    $destinationPath = public_path('storage/attachments/'); // upload path
        ...
        if (!$error->status) {
            ...
            $touserdata = DB::table('users')->where('id', $request['id'])->first();
            $fromusersdata = DB::table('users')->where('id', Auth::user()->id)->first();
            $msg = htmlentities(trim($request['message']), ENT_QUOTES, 'UTF-8');
            if($touserdata->active_status == 0){
                Mail::send('email.offilineUserMessageToEmail', ['msg' => $msg,'fromusername' => $fromusersdata->fname,'tousername'=> $touserdata->fname], function($message) use($touserdata,$fromusersdata){
                    $message->to($touserdata->email);
                    $message->subject($fromusersdata->fname.' has sent you a message');
                });                 
            }
    ...
    public function deleteConversation(Request $request)
    {
        ...
        $delete = 1;
        $user_id = $request['id'];
        $dataa = Message::where('from_id', Auth::user()->id)->where('to_id', $user_id)
                    ->orWhere('from_id', $user_id)->where('to_id', Auth::user()->id)->get();
        foreach ($dataa as $msg) {
            // delete from database
            $delete = DB::table('ch_messages')->where('id',$msg->id)->update($array);
        }
    ...
    public function deleteMessage(Request $request)
    {
        ...
        // delete
        $delete = DB::table('ch_messages')->where('id',$request["id"])->update($array);
        
        // send the response
        return Response::json([
            'deleted' => $delete ? 1000 : 0,
        ], 200);
    }
